const PROJECT_GAUGE_RADIUS = 43,
  PROJECT_GAUGE_CIRCUMFERENCE = 2 * Math.PI * PROJECT_GAUGE_RADIUS;
function projectGaugeOffset(value) {
  const p = Math.max(0, Math.min(100, Number(value) || 0));
  return PROJECT_GAUGE_CIRCUMFERENCE * (1 - p / 100);
}
const KEY = "maskanProjectsV1";
let projects = window.TOC_EMBEDDED_EXPORT
  ? structuredClone(MASKAN_PROJECTS)
  : JSON.parse(localStorage.getItem(KEY) || "null") || MASKAN_PROJECTS;
...
let detailsScrollY = 0,
  detailsOpener = null;
function consolidateSandProjects(list) {
  const source = MASKAN_PROJECTS.find((p) => p.name.includes("الرمال")),
    sand = list.filter((p) => p.name.includes("الرمال"));
  if (!source || !sand.length) return list;
  const combined = {
    ...structuredClone(source),
    id: 43,
    code: "",
    name: "منازل الرمال",
    uploadedFiles: sand.flatMap((p) => p.uploadedFiles || []),
  };
  let inserted = false;
  return list.flatMap((p) => {
    if (!p.name.includes("الرمال")) return [p];
    if (inserted) return [];
    inserted = true;
    return [combined];
  });
}
projects = consolidateSandProjects(projects);
localStorage.setItem(KEY, JSON.stringify(projects));
const uniq = (k) =>
  [...new Set(projects.map((p) => p[k]).filter(Boolean))].sort();
function fillFilters() {
  uniq("type").forEach((v) =>
    typeFilter.insertAdjacentHTML("beforeend", `<option>${v}</option>`),
  );
  uniq("owner").forEach((v) =>
    ownerFilter.insertAdjacentHTML("beforeend", `<option>${v}</option>`),
  );
}
function dashboard() {
  const total = projects.length || 1,
    avg = Math.round(projects.reduce((s, p) => s + p.progress, 0) / total),
    done = projects.filter((p) => p.status === "مكتمل").length,
    active = projects.filter((p) => p.status === "قيد التنفيذ").length,
    early = projects.filter((p) => p.progress < 25).length;
  document
    .querySelector("#radial")
    .style.setProperty("--deg", avg * 3.6 + "deg");
  document.querySelector("#radialValue").textContent = avg + "%";
  document.querySelector("#portfolioLabel").textContent =
    avg >= 80 ? "أداء متقدم" : avg >= 60 ? "أداء جيد" : "يحتاج متابعة";
  document.querySelector("#completePct").textContent =
    Math.round((done / total) * 100) + "%";
  document.querySelector("#activePct").textContent =
    Math.round((active / total) * 100) + "%";
  document.querySelector("#earlyPct").textContent =
    Math.round((early / total) * 100) + "%";
  const hi = projects.filter((p) => p.progress >= 75).length;
  document.querySelector("#kpis").innerHTML = [
    ["إجمالي المشاريع", projects.length, "كامل المحفظة"],
    ["مشاريع مكتملة", done, Math.round((done / total) * 100) + "% من الإجمالي"],
    ["قيد التنفيذ", active, "مشروع نشط"],
    ["إنجاز 75% فأكثر", hi, Math.round((hi / total) * 100) + "% من المحفظة"],
  ]
    .map(
      (x) =>
  ...
  projects.forEach(
    (p) =>
      (types[p.type || "غير محدد"] = (types[p.type || "غير محدد"] || 0) + 1),
  );
  const max = Math.max(1, ...Object.values(types));
  document.querySelector("#typeChart").innerHTML = Object.entries(types)
    .sort((a, b) => b[1] - a[1])
    .map(
      ([k, v]) =>
        `<div class="type-row"><span>${k}</span><div class="type-track"><i style="width:${(v / max) * 100}%"></i></div><b>${v}</b></div>`,
    )
    .join("");
  const owners = {};
  projects.forEach(
    (p) =>
      (owners[p.owner || "مالك غير محدد"] =
        (owners[p.owner || "مالك غير محدد"] || 0) + 1),
  );
  const ownerMax = Math.max(1, ...Object.values(owners));
  document.querySelector("#ownerChart").innerHTML = Object.entries(owners)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 6)
    .map(
      ([k, v]) =>
        `<div class="type-row owner-row"><span title="${k}">${k}</span><div class="type-track"><i style="width:${(v / ownerMax) * 100}%"></i></div><b>${v}</b></div>`,
    )
    .join("");
  const stages = [
    ["البداية", "0% – 25%", (p) => p.progress <= 25],
    ["التنفيذ الأولي", "26% – 50%", (p) => p.progress > 25 && p.progress <= 50],
    [
      "التنفيذ المتقدم",
      "51% – 75%",
      (p) => p.progress > 50 && p.progress <= 75,
    ],
    ["قرب التسليم", "76% – 99%", (p) => p.progress > 75 && p.progress < 100],
    ["مكتمل", "100%", (p) => p.progress >= 100],
  ];
  document.querySelector("#stageChart").innerHTML = stages
    .map(([name, range, test]) => {
      const count = projects.filter(test).length,
        pct = Math.round((count / total) * 100);
      return `<div class="document-row stage-row"><div><b>${name}</b><small>${range} • ${count} مشروع</small></div><div class="document-track"><i style="width:${pct}%"></i></div><strong>${pct}%</strong></div>`;
    })
    .join("");
}
...
function projectLastUpdateAt(project) {
  ...
  if (Array.isArray(project?.updateLog))
    project.updateLog.forEach((entry) => {
      if (entry?.at) values.push(entry.at);
    });
  return (
    values
...
function latestProjectChangeMeta() {
  ...
  projects.forEach((project) => {
    const at = projectLastUpdateAt(project);
    if (!at) return;
    const date = new Date(at);
    if (!latest || date > new Date(latest.at))
      latest = {
        at,
        editor:
          (project.updateLog || [])
            .slice()
            .sort((a, b) => new Date(b.at) - new Date(a.at))[0]?.editor || "",
        action: "تعديل مشروع",
...
function getPlatformChangeMeta() {
  if (window.MASKAN_EXPORTED_CHANGE_META?.at)
    return window.MASKAN_EXPORTED_CHANGE_META;
  if (!window.TOC_EMBEDDED_EXPORT) {
    try {
      const stored = JSON.parse(
        localStorage.getItem(CHANGE_META_KEY) || "null",
      );
      if (stored?.at) return stored;
    } catch (_) {}
  }
  const latest = latestProjectChangeMeta();
  if (latest) return latest;
  const exportedAt = document.querySelector(
    'meta[name="toc-exported-at"]',
  )?.content;
  return exportedAt
    ? { at: exportedAt, action: "تصدير النسخة المحدثة", details: [] }
    : null;
}
...
function describePlatformChange(meta) {
  ...
  const project = meta.projectName
    ? `${meta.projectName}${meta.projectCode ? ` #${meta.projectCode}` : ""}`
    : "";
  const details =
    Array.isArray(meta.details) && meta.details.length
      ? ` — ${meta.details.slice(0, 4).join("، ")}`
      : meta.summary
        ? ` — ${meta.summary}`
        : "";
...
function renderPlatformLastUpdate() {
  const dateEl = document.querySelector("#platformLastUpdate");
  if (!dateEl) return;
  const meta = getPlatformChangeMeta();
  dateEl.textContent = meta?.at
    ? formatVisibleUpdateDate(meta.at)
    : "لم يُسجّل تعديل بعد";
...
const stageTests = [
  ...
  (p) => p.progress > 50 && p.progress <= 75,
  (p) => p.progress > 75 && p.progress < 100,
  (p) => p.progress >= 100,
];
function current() {
  ...
  return projects.filter((p) => {
    const stageMatch = activeStage === null || stageTests[activeStage](p);
    return (
      stageMatch &&
      (t === "الكل" || p.type === t) &&
      (o === "الكل" || p.owner === o) &&
      `${p.name} ${p.code} ${p.location} ${p.owner}`.includes(q)
    );
  });
}
function render() {
...
function renderUpdateLog(project) {
  const log = Array.isArray(project.updateLog)
    ? project.updateLog.slice().reverse()
    : [];
  return `<section class="project-updates-section"><div class="details-section-title"><div><span class="details-section-icon updates-icon">↻</span><div><small>التوثيق</small><h3>سجل التحديثات</h3></div></div><b>${log.length}</b></div>${
    log.length
      ? `<div class="updates-timeline">${log
          .slice(0, 50)
          .map(
            (entry) =>
              `<article class="update-entry"><div class="update-marker"></div><div class="update-entry-head"><time>${formatUpdateDate(entry.at)}</time>${entry.editor ? `<span>${safeText(entry.editor)}</span>` : ""}</div><div class="update-changes">${(entry.changes || []).map((change) => `<div><b>${safeText(change.label || auditFieldLabels[change.field] || change.field || "تحديث")}</b><p><del>${safeText(formatAuditValue(change.before))}</del><span>←</span><ins>${safeText(formatAuditValue(change.after))}</ins></p></div>`).join("")}</div></article>`,
          )
          .join("")}</div>`
      : '<p class="details-empty-state">لم تُسجّل تعديلات على بيانات المشروع بعد.</p>'
  }</section>`;
}
function show(p) {
  ...
  else details.style.removeProperty("--details-project-image");
  document.querySelector("#detailsBody").innerHTML =
    `<span class="eyebrow dark">لوحة المشروع</span><h2>${p.name}</h2><div class="details-list"><div><small>رقم المشروع</small><b>${p.code || "—"}</b></div><div><small>الحالة</small><b>${p.status}</b></div><div><small>نسبة الإنجاز</small><b>${p.progress}%</b></div><div><small>نوع المشروع</small><b>${p.type || "—"}</b></div><div><small>المالك</small><b>${p.owner || "—"}</b></div><div><small>مدير المشروع</small><b>${p.manager || "—"}</b></div><div><small>الموقع</small><b>${p.location || "—"}</b></div><div><small>المدة</small><b>${p.startDate || "—"} — ${p.endDate || "—"}</b></div><div><small>انتهاء العقد</small><b>${formatProjectDate(p.contractEndDate)}</b></div><div class="details-last-update"><small>آخر تحديث للمشروع</small><b>${projectLastUpdateAt(p) ? formatVisibleUpdateDate(projectLastUpdateAt(p)) : "غير مسجل"}</b></div></div>${renderMissingItems(p)}${renderUpdateLog(p)}<h3>روابط مجلدات المشروع</h3><div class="docs">${
      Object.entries(docs)
        .map(
          ([k, v]) =>
...
grid.addEventListener("click", (e) => {
  ...
  if (id) show(projects.find((p) => String(p.id) === id));
});
[search, typeFilter, ownerFilter].forEach((x) =>
...
function renderAlertCenter() {
  ...
  const isOpenProject = (project) =>
    project.status !== "مكتمل" && Number(project.progress) < 100;
  const withDays = (project, field) => {
    const date = parseProjectDate(project[field]);
    return date ? { project, date, days: daysFromToday(date) } : null;
  };
  const delayed = projects
    .map((project) => withDays(project, "endDate"))
    .filter((item) => item && isOpenProject(item.project) && item.days < 0)
    .map((item) => ({ ...item, days: Math.abs(item.days) }))
  ...
  const ending = projects
    ...
    .filter(
      (item) =>
        item &&
        isOpenProject(item.project) &&
        item.days >= 0 &&
        item.days <= 30,
    )
    .sort((a, b) => a.days - b.days);
  const empty = (message) =>
    `<div class="alert-empty"><span>✓</span><p>${message}</p></div>`;
  const remainingLabel = (days) =>
    days === 0
      ? "ينتهي اليوم"
      : `متبقي ${days} ${days === 1 ? "يوم" : "يومًا"}`;
  delayedContainer.innerHTML = delayed.length
    ? delayed
        .map(
          ({ project, days }) =>
            `<button type="button" class="alert-item delayed" data-alert-project="${project.id}"><span class="alert-item-code">#${project.code || "—"}</span><div><b>${safeText(project.name)}</b><small>${safeText(project.location || "الموقع غير محدد")}</small></div><strong>متأخر ${days} ${days === 1 ? "يوم" : "يومًا"}</strong></button>`,
        )
        .join("")
    : empty("لا توجد مشاريع متأخرة وفق التواريخ المسجلة.");
  endingContainer.innerHTML = ending.length
    ? ending
        .map(
          ({ project, days }) =>
        ...
        .join("")
    : empty("لا توجد مشاريع قريبة من الانتهاء خلال الثلاثين يومًا القادمة.");
  ...
  document.querySelector("#alertsTotal").textContent =
    delayed.length + ending.length;
}
document.querySelector("#alerts")?.addEventListener("click", (event) => {
  const target = event.target.closest("[data-alert-project]");
  if (!target) return;
  const project = projects.find(
    (item) => String(item.id) === String(target.dataset.alertProject),
  );
...
function clearDashboardSelection() {
  ownerRows.forEach((row) => row.classList.remove("selected"));
  stageRows.forEach((row) => row.classList.remove("selected"));
}
...
ownerRows.forEach((row) => {
  ...
  const selectOwner = () => {
    ...
    search.value = "";
    typeFilter.value = "الكل";
    ownerFilter.value = row.querySelector("span").textContent.trim();
    clearDashboardSelection();
    row.classList.add("selected");
    showFilteredProjects();
  };
  row.addEventListener("click", selectOwner);
...
stageRows.forEach((row, index) => {
  ...
  row.addEventListener("click", selectStage);
  row.addEventListener("keydown", (e) => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      selectStage();
...
if (publicBrand) {
  publicBrand.querySelector("b").textContent = "سحابة المكتب الفني";
  publicBrand.querySelector("small").textContent =
    "Technical Office Cloud (TOC)";
}
...
const iconObserver = new MutationObserver(() =>
  document
    .querySelectorAll(".docs:not(.uploaded-docs) .doc")
    .forEach((link) => {
      const label = link.querySelector("b")?.textContent.trim(),
        icon = link.querySelector(":scope>span");
      if (icon) icon.dataset.icon = folderIcons[label] || "📂";
    }),
);
const detailsBodyForIcons = document.querySelector("#detailsBody");
if (detailsBodyForIcons)
  iconObserver.observe(detailsBodyForIcons, { childList: true, subtree: true });

const projectBackgrounds = {
  88: "../images/project-images/p88.webp",
  90: "../images/project-images/p90.webp",
  92: "../images/project-images/p92.webp",
  96: "../images/project-images/p96.webp",
  98: "../images/project-images/p98.webp",
  100: "../images/project-images/p100.webp",
  102: "../images/project-images/p102.webp",
  104: "../images/project-images/p104.webp",
  106: "../images/project-images/p106.webp",
  108: "../images/project-images/p108.webp",
  110: "../images/project-images/p110.webp",
  112: "../images/project-images/p112.webp",
  114: "../images/project-images/p114.webp",
  116: "../images/project-images/p116.webp",
  118: "../images/project-images/p118.webp",
  120: "../images/project-images/p120.webp",
  122: "../images/project-images/p122.webp",
  124: "../images/project-images/p124.webp",
  126: "../images/project-images/p126.webp",
  128: "../images/project-images/p128.webp",
  130: "../images/project-images/p130.webp",
  132: "../images/project-images/p132.webp",
  136: "../images/project-images/p136.webp",
  138: "../images/project-images/p138.webp",
  140: "../images/project-images/p140.webp",
  142: "../images/project-images/p142.webp",
  144: "../images/project-images/p144.webp",
  146: "../images/project-images/p146.webp",
  148: "../images/project-images/p148.webp",
  150: "../images/project-images/p150.webp",
  152: "../images/project-images/p152.webp",
  154: "../images/project-images/p154.webp",
  156: "../images/project-images/p156.webp",
  158: "../images/project-images/p158.webp",
  160: "project-card-bg.webp",
};
const namedProjectBackgrounds = [
  ["العقيق (3) فلتين تجارية", "../images/project-images/p98-commercial.webp"],
  ["العقيق (3) فلة خاصة", "../images/project-images/p98-private.webp"],
  ["مقر الشركة", "../images/project-images/head-office.webp"],
  ["المعيزيلة", "../images/project-images/al-muaizilah.webp"],
  ["الملز كدا الاعمار", "../images/project-images/al-malaz-kada.webp"],
  ["الملز بلوك شرقي", "../images/project-images/al-malaz-east.webp"],
  ["الملز بلوك غربي", "../images/project-images/al-malaz-west.webp"],
  ["منازل الرمال", "../images/project-images/sand-homes.webp"],
];
...
function applyProjectCardBackground() {
  document.querySelectorAll("#grid .card").forEach((card) => {
    const code = (
        card.querySelector(".card-head small")?.textContent || ""
      ).replace(/\D/g, ""),
      name = card.querySelector("h3")?.textContent.trim() || "";
    const named = namedProjectBackgrounds.find(([label]) =>
      name.includes(label),
    );
    const image = named?.[1] || projectBackgrounds[code] || "";
    if (!image) {
      ...
      card.style.removeProperty("--project-card-image");
      delete card.dataset.bgImage;
      return;
    }
    card.classList.add("project-image-card");
    if (card.dataset.bgImage === image) return;
    card.dataset.bgImage = image;
    card.classList.remove("image-ready");
    card.classList.add("image-loading");
    const loader = new Image();
    loader.onload = () => {
      if (card.dataset.bgImage !== image) return;
      card.style.setProperty("--project-card-image", `url("${image}")`);
      card.classList.remove("image-loading");
      requestAnimationFrame(() => card.classList.add("image-ready"));
    };
    loader.onerror = () => {
      card.classList.remove("image-loading");
      card.style.removeProperty("--project-card-image");
...
if (grid) {
  projectGridObserver.observe(grid, { childList: true });
  applyProjectCardBackground();
}

document.head.insertAdjacentHTML(
  "beforeend",
  '<link rel="stylesheet" href="logo-transparent.css">',
);
...
intro.innerHTML = `<div class="toc-intro-inner">
  <div class="toc-intro-emblem" aria-hidden="true">
    ...
    <span class="toc-intro-logo-shell"><span class="toc-intro-logo"><img src="../images/maskan-logo.png" alt=""></span></span>
  </div>
  ...
  const aboutObserver = new IntersectionObserver(
    (entries) => {
      ...
      if (entry.isIntersecting) {
        aboutSection.classList.remove("values-visible");
        requestAnimationFrame(() =>
          requestAnimationFrame(() =>
            aboutSection.classList.add("values-visible"),
          ),
        );
      } else {
        aboutSection.classList.remove("values-visible");
...
if (liveBadge) {
  ...
  clock.className = "performance-clock";
  clock.innerHTML = '<time aria-label="الوقت الحالي"></time><span></span>';
  liveBadge.before(status);
  status.append(liveBadge, clock);
  const updateClock = () => {
    const now = new Date();
    ...
    clock.querySelector("span").textContent = new Intl.DateTimeFormat(
      "ar-SA-u-ca-gregory-nu-latn",
      {
        timeZone: "Asia/Riyadh",
        weekday: "long",
...
document.head.insertAdjacentHTML(
  "beforeend",
  '<link rel="stylesheet" href="professional-animations.css">',
);
...
function tweenNumber(element, target, duration = 950, suffix = "") {
  ...
  const tick = (now) => {
    ...
    element.textContent = Math.round(target * easeOut(p)) + suffix;
...
function resetAnalyticsMotion() {
  if (!analytics) return;
  analytics.classList.remove("analytics-in-view");
  analytics
    .querySelectorAll(".type-track i,.document-track i")
    .forEach((bar) => {
      if (!bar.dataset.targetWidth) bar.dataset.targetWidth = bar.style.width;
      bar.style.width = "0";
    });
  const radial = document.querySelector("#radial");
  if (radial) {
    if (!radial.dataset.targetDeg)
      radial.dataset.targetDeg = radial.style.getPropertyValue("--deg");
    radial.style.setProperty("--deg", "0deg");
  }
}
function playAnalyticsMotion() {
  if (!analytics) return;
  resetAnalyticsMotion();
  requestAnimationFrame(() => {
    analytics.classList.add("analytics-in-view");
    analytics
      .querySelectorAll(".type-track i,.document-track i")
      .forEach((bar, index) =>
        setTimeout(
    ...
    const kpis = [...analytics.querySelectorAll("#kpis article>b")];
    kpis.forEach((b, index) => {
      const value = parseInt(b.textContent) || 0;
      b.textContent = "0";
      setTimeout(() => tweenNumber(b, value, 800), index * 90);
    });
    const radial = document.querySelector("#radial"),
      radialValue = document.querySelector("#radialValue");
    if (radial && radialValue) {
      const value = parseInt(radialValue.textContent) || 0;
      radialValue.textContent = "0%";
      setTimeout(() => {
        radial.style.setProperty(
          "--deg",
          radial.dataset.targetDeg || value * 3.6 + "deg",
        );
        tweenNumber(radialValue, value, 1050, "%");
      }, 130);
    }
    ["completePct", "activePct", "earlyPct"].forEach((id, index) => {
      const el = document.getElementById(id),
        value = parseInt(el?.textContent) || 0;
      if (el) {
        el.textContent = "0%";
        setTimeout(() => tweenNumber(el, value, 800, "%"), 220 + index * 100);
      }
    });
  });
}
if (analytics) {
  ...
  const analyticsObserver = new IntersectionObserver(
    (entries) => {
      const entry = entries[0];
      if (entry.isIntersecting) playAnalyticsMotion();
      else resetAnalyticsMotion();
    },
    { threshold: 0.18 },
  );
  analyticsObserver.observe(analytics);
}
  ...
  (entries) =>
    entries.forEach((entry) => {
      ...
      const progress = Math.max(
        0,
        Math.min(100, Number(gauge?.dataset.progress) || 0),
      );
      if (entry.isIntersecting) {
        ...
        if (circle)
          requestAnimationFrame(() => {
            circle.style.strokeDashoffset = String(
              projectGaugeOffset(progress),
            );
          });
        if (number) {
          number.textContent = "0%";
          setTimeout(() => tweenNumber(number, progress, 850, "%"), 140);
        }
      } else {
...
if (grid) {
  new MutationObserver(prepareProjectCards).observe(grid, { childList: true });
  prepareProjectCards();
}
...
show = function (project) {
  ...
  requestAnimationFrame(() =>
    requestAnimationFrame(() => details.classList.add("details-animate")),
  );
};
...
function restoreDetailsScrollPosition() {
  ...
  requestAnimationFrame(() => {
    ...
    if (detailsOpener && detailsOpener.isConnected) {
      try {
        detailsOpener.focus({ preventScroll: true });
      } catch (_) {
        /* متصفح قديم */
      }
    }
...
if (details) {
  details.addEventListener("cancel", (event) => {
    event.preventDefault();
    closeDetailsAnimated(event);
  });
  details.addEventListener("close", () => {
    clearTimeout(detailsCloseTimer);
    details.classList.remove("details-closing", "details-animate");
    if (detailsShouldRestore) {
      detailsShouldRestore = false;
      restoreDetailsScrollPosition();
    }
  });
}
...
const navObserver = new IntersectionObserver(
  ...
  { rootMargin: "-25% 0px -55%", threshold: [0, 0.1, 0.3, 0.6] },
);
...
function resolveHeroProjectImage(project) {
  ...
  const named = namedProjectBackgrounds.find(([label]) =>
    projectName.includes(label),
  );
  return (
    named?.[1] || projectBackgrounds[String(project?.code || "").trim()] || ""
  );
}
function initHeroProjectSlideshow() {
  if (!hero || hero.querySelector(".hero-project-slideshow")) return;
  const images = [
    ...new Set(projects.map(resolveHeroProjectImage).filter(Boolean)),
  ];
  ...
  stage.innerHTML =
    '<div class="hero-project-slide"></div><div class="hero-project-slide"></div>';
  hero.prepend(stage);
  ...
  let activeLayer = 0,
    ...
    intervalId = 0,
    startDelayId = 0;
  const assignImage = (layer, source) => {
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.hidden) stop();
      ...
      clearTimeout(startDelayId);
      startDelayId = window.setTimeout(start, 3000);
    }
  });
}
...
function initProjectShowcase() {
  ...
  const slides = projects
    .map((project) => ({ project, image: resolveHeroProjectImage(project) }))
  ...
  dotsEl.innerHTML = slides
    .map(
      (_, i) => `<button type="button" aria-label="المشروع ${i + 1}"></button>`,
    )
    .join("");
  ...
  const restartLine = () => {
    ...
    if (!motionReduced) shell.classList.add("is-running");
  };
  ...
  const update = (nextIndex, instant = false) => {
    ...
    restartLine();
    const preload = slides[(index + 1) % slides.length]?.image;
    if (preload) {
      const img = new Image();
      img.src = preload;
    }
  ...
  const start = () => {
    stop();
    if (slides.length > 1 && !motionReduced && !paused)
      timer = window.setInterval(() => update(index + 1), 3000);
  };
  const navigate = (next) => {
    update(next);
    start();
  ...
  document
    .querySelector("#showcasePrev")
    ?.addEventListener("click", () => navigate(index - 1));
  document
    .querySelector("#showcaseNext")
    ?.addEventListener("click", () => navigate(index + 1));
...
function initCompanyAchievements() {
  ...
  try {
    stored =
      JSON.parse(localStorage.getItem("maskanCompanyStatsV1") || "null") || {};
  } catch (_) {
    stored = {};
  }
  const stats = {
    ...defaults,
    ...(window.MASKAN_EXPORTED_COMPANY_STATS || {}),
    ...stored,
  };
  const values = {
  ...
  section
    ...
    .forEach((card, index) =>
      card.style.setProperty("--achievement-delay", index * 90 + "ms"),
    );
  const reveal = () => {
    ...
    section.querySelectorAll(".achievement-value").forEach((el, index) => {
      ...
      setTimeout(
        () => {
          if (typeof tweenNumber === "function")
            tweenNumber(el, target, 1000, suffix);
...
function initPlatformClosingAnimation() {
  ...
  const showCloseFallback = (overlay) => {
    ...
    overlay.innerHTML = `<div class="toc-outro-fallback" role="dialog" aria-modal="true" aria-label="تم إنهاء جلسة المنصة">
      <img src="../images/maskan-logo.png" alt="شعار مسكن الكيان للمقاولات">
    ...
    overlay
      .querySelector("[data-outro-return]")
      ?.addEventListener("click", () => restorePlatform(overlay), {
        once: true,
      });
  };
